...
import type { Database } from '@clinro/types/schema'
  ...
  await db
    ...
    .where(
      and(
        ...
        lt(schema.loginAttempts.createdAt, windowStart.toISOString())
      )
    )
...
export class RateLimiter {
  ...
  async check(
    ...
    await this.db
      ...
      .where(
        and(
          eq(schema.loginAttempts.identifier, key),
          lt(schema.loginAttempts.createdAt, windowStart.toISOString())
        )
      )
    ...
    const attemptsInWindow = recentAttempts.filter(
      (a) => (a.createdAt ?? '') >= windowStart.toISOString()
    )
  ...
  async reset(key: string): Promise<void> {
    ...
    await this.db.delete(schema.loginAttempts).where(eq(schema.loginAttempts.identifier, key))
  }
}
